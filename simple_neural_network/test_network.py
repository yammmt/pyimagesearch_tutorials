...
for imagePath in paths.list_images(args["test_images"]):
    # load the image, resize it to a fixed 32x32 pixels (ignoring aspect ratio),
    ...
    # draw the clas and probability on the test image and display it to our screen
    label = "{}: {:.2f}%".format(CLASSES[prediction], probs[prediction]*100.0)
