...
# perform edge detection, then perform a dilation + erosion to close gaps in
...
# sort the contours from left-to-right and initialize the `pixels_per_metric`
...
for c in cnts:
    ...
    # if: OpenCV 2.4 or 3.0
    box = cv2.cv.BoxPoints(box) if imutils.is_cv2() else cv2.boxPoints(box)
    box = np.array(box, dtype="int")

    # order the points in the contour such that they appear in to-left,
    # top-right, bottom-right, and bottom-left order. then draw the outline of
    ...
    # unpack the ordered bounding box, then compute the midpoint between the
    ...
    # by the midpoint between the top-right and bottom-right
